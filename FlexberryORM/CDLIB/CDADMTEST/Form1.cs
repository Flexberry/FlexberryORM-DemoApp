namespace CDADMTEST
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            cdda.Track.Add(new Track() {Name="My strange love", Author=prs1, Singer=prs2, Length=new Random().Next(100,600)}); 
            cdda.Track.Add(new Track() {Name = "Stupid is as stupid does", Author = prs2, Singer = prs1, Length = new Random().Next(100, 600) });
            ...
            objstoupdlist.AddRange(new ICSSoft.STORMNET.DataObject[] { ctr1, ctr2, ctr3, prs1, prs2, pblshr1, pblshr2, cdda, cddd }); // Just put all the objects that needs to be persisted in one array 
            ...
            try
            {
                ...
                DataServiceProvider.DataService.UpdateObjects(ref objstoupd); // Flexberry ORM persists dataobjects according to their statuses. Flexberry automatically resolves dependencies between dataobjects. All queries runs in one transaction. 
                // 1 UpdateObjects call = 1 DB transaction.
                MessageBox.Show("OK!");
            }
            catch (Exception exc)
            {
                MessageBox.Show(string.Format("Something wrong: {0}", exc.ToString()));
            }
        ...
        {            
            ...
            cdda.Name = "Blablabla " + DateTime.Now.ToString(); //Let's try to change a value of one loaded property. You can change only loaded properties. If you change not-loaded property, you'll get an error while persisting.
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            lcs.LoadingTypes = new Type[] { typeof(CDDA), typeof(CDDD), typeof(DVD) }; // It's a limitation on descendants of class CD. Only view-class and it's descendants can be loaded.
            lcs.LimitFunction = ld.GetFunction(ld.funcEQ, new VariableDef(ld.StringType, "Publisher.Country.Name"), "USA"); // Limitation on property value. You can use only properties in view. If the property doesn't exist in a view, you'll get an error while loading.
            lcs.ColumnsSort = new ColumnsSortDef[] { new ColumnsSortDef("Name", ICSSoft.STORMNET.Business.SortOrder.Asc) }; //Ordering of dataobjects in result array
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            ...
            DateTime starttime = DateTime.Now; //Take timeshot
            DataServiceProvider.DataService.UpdateObject(aggregator); // All objects will be saved according to composition semantic

            MessageBox.Show(string.Format("Time taken for persistence: {0}", DateTime.Now.Subtract(starttime)));
        }
        ...
        private void button21_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show(string.Format("Time taken for loading: {0}", DateTime.Now.Subtract(starttime)));
